...
export default defineConfig({
	...
	build: {
		 ...
		 *
		 * Vite inlines any asset under 4 KB, which caught one subset of Hanken
		 * Grotesk (Cyrillic, 1.6 KB). The app's CSP has no `font-src`, so those
		 * fall to `default-src 'self'` and the browser refused to load it: a
		 * Cyrillic display name rendered in the fallback face and logged a CSP
		 * violation on every page. Allowing `data:` for fonts would fix it too,
		 * and this keeps the policy as narrow as it is everywhere else.
		 *
		 * Returning undefined for everything else leaves Vite's default limit in
		 * place, so the icons and small images inline as before.
		 */
	...
	plugins: [
		...
		sveltekit({
			...
			...(DEMO
				? {
						// appTemplate: app.html with the boot screen injected, generated by
						// scripts/demo-build.ts from the same source the server build uses.
						files: {
							routes: '.demo/routes',
							appTemplate: '.demo/app.html',
							hooks: { server: 'src/demo-hooks.server' }
						},
